...
def loop(start_date, end_date, obslimit, info_text, RA, Decl, LON, LAT):    #going trough data
    ...
    while start_date <= end_date:
        ...
        if sol_altitude < -6 and altitude > obslimit:
            # Note rising time
            if len(arvo) == 0:
                text = 'Rising ' + datetime.time(start_date).strftime("%H:%M") + ' at altitude ' + f'{altitude:.2f}°';
    ...
    if len(arvo) > 1:
        ...
        if max_index == 0 or max_index + 1 == len(arvo):
            ...
            text = ' Transit ' + datetime.time(pvm[max_index]).strftime("%H:%M") + ' at altitude ' + f'{max(arvo):.2f}°';
        info_text.append(text);
        text = ' Setting ' + datetime.time(pvm[-1]).strftime("%H:%M") + ' at altitude ' + f'{arvo[-1]:.2f}°';
        info_text.append(text);
...
def figure_mo(arvo, pvm, obslimit, info_text, id_mo, RA, Decl, LON, LAT):
    ...
    # graphic plotting
    ax.plot(df3);
    ax.set_title('Night Sky Object ' + id_mo + ' RA / Dec ' + "{:.4f}".format(RA) + ', ' + "{:.4f}".format(Decl) + ' at ' + name + ' Lat / Lon '
                 + "{:.4f}".format(LAT) + '°' + "{:.4f}".format(LON) + '° on ' + start_date.strftime("%Y-%m-%d"));
    ax.set_xlabel('time (mm-dd hh)');                      # labels
    ax.set_ylabel('Object altitude (°)');
...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "Visibility on Slooh Observatories"))
        self.submit.setText(_translate("MainWindow", "Submit"))
        self.label.setText(_translate("MainWindow", "No image saving"))
    ...
    def checkboxChangedAction(self, state):
        ...
        if (QtCore.Qt.Checked == state):
            self.label.setText("Image saving on")
            fs = True
        else:
            self.label.setText("No image saving")
            fs = False
